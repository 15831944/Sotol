...
#error "You are defining LOG_IMPLEMENTATION. This is for older versions of Loguru. You should now instead include loguru.cpp (or build it and link with it)"
#endif
...
// Semantic versioning. Loguru version can be printed with printf("%d.%d.%d", LOG_VERSION_MAJOR, LOG_VERSION_MINOR, LOG_VERSION_PATCH);
...
namespace log
{
	...
	LOG_EXPORT
		Text textprintf(LOG_FORMAT_STRING_TYPE format, const Args&... args) {
...
#undef MAX
	...
	*/
	...
	LOG_EXPORT extern bool      g_colorlogtostderr; // True by default.
	LOG_EXPORT extern unsigned  g_flush_interval_ms; // 0 (unbuffered) by default.
	LOG_EXPORT extern bool      g_preamble_header; // Prepend each log start by a descriptions line with all columns name? True by default.
	LOG_EXPORT extern bool      g_preamble; // Prefix each log line with date, time etc? True by default.
	...
	*/
	LOG_EXPORT extern Verbosity g_internal_verbosity;

	// Turn off individual parts of the preamble
	LOG_EXPORT extern bool      g_preamble_date; // The date field
	LOG_EXPORT extern bool      g_preamble_time; // The time of the current day
	LOG_EXPORT extern bool      g_preamble_uptime; // The time since init call
	LOG_EXPORT extern bool      g_preamble_thread; // The logging thread
	LOG_EXPORT extern bool      g_preamble_file; // The file from which the log originates from
	LOG_EXPORT extern bool      g_preamble_verbose; // The verbosity field
	LOG_EXPORT extern bool      g_preamble_pipe; // The pipe symbol right before the message
	...
	*/
	...
	// Internal functions
	LOG_EXPORT
		void vlog(Verbosity verbosity, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, fmt::format_args args);
	LOG_EXPORT
		void raw_vlog(Verbosity verbosity, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, fmt::format_args args);

	// Actual logging function. Use the LOG macro instead of calling this directly.
	template <typename... Args>
	LOG_EXPORT
		void log(Verbosity verbosity, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, const Args &... args) {
	...
	template <typename... Args>
	LOG_EXPORT
		void raw_log(Verbosity verbosity, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, const Args &... args) {
...
#if LOG_USE_FMTLIB
	LOG_EXPORT
		LOG_NORETURN void vlog_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, fmt::format_args);
	template <typename... Args>
	LOG_EXPORT
		LOG_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOG_FORMAT_STRING_TYPE format, const Args&... args) {
	...
	template<>        inline Text format_value(const char& v) { return textprintf(LOG_FMT(c), v); }
	template<>        inline Text format_value(const int& v) { return textprintf(LOG_FMT(d), v); }
	template<>        inline Text format_value(const unsigned int& v) { return textprintf(LOG_FMT(u), v); }
	template<>        inline Text format_value(const long& v) { return textprintf(LOG_FMT(lu), v); }
	template<>        inline Text format_value(const unsigned long& v) { return textprintf(LOG_FMT(ld), v); }
	template<>        inline Text format_value(const long long& v) { return textprintf(LOG_FMT(llu), v); }
	template<>        inline Text format_value(const unsigned long long& v) { return textprintf(LOG_FMT(lld), v); }
	template<>        inline Text format_value(const float& v) { return textprintf(LOG_FMT(f), v); }
	template<>        inline Text format_value(const double& v) { return textprintf(LOG_FMT(f), v); }
	...
	*/
	...
	// Colors
	LOG_EXPORT const char* terminal_black();
	LOG_EXPORT const char* terminal_red();
	LOG_EXPORT const char* terminal_green();
	LOG_EXPORT const char* terminal_yellow();
	LOG_EXPORT const char* terminal_blue();
	LOG_EXPORT const char* terminal_purple();
	LOG_EXPORT const char* terminal_cyan();
	LOG_EXPORT const char* terminal_light_gray();
	LOG_EXPORT const char* terminal_light_red();
	LOG_EXPORT const char* terminal_white();

	// Formating
	LOG_EXPORT const char* terminal_bold();
	LOG_EXPORT const char* terminal_underline();

	// You should end each line with this!
	LOG_EXPORT const char* terminal_reset();
...
#define ERROR_CONTEXT(descr, data)                                             \
	...
	*/
	...
	LOG_EXPORT Text ec_to_text(const char* data);
	LOG_EXPORT Text ec_to_text(char data);
	LOG_EXPORT Text ec_to_text(int data);
	LOG_EXPORT Text ec_to_text(unsigned int data);
	LOG_EXPORT Text ec_to_text(long data);
	LOG_EXPORT Text ec_to_text(unsigned long data);
	LOG_EXPORT Text ec_to_text(long long data);
	LOG_EXPORT Text ec_to_text(unsigned long long data);
	LOG_EXPORT Text ec_to_text(float data);
	LOG_EXPORT Text ec_to_text(double data);
	LOG_EXPORT Text ec_to_text(long double data);
	LOG_EXPORT Text ec_to_text(EcHandle);
...
#if LOG_WITH_STREAMS
#ifndef LOG_HAS_DECLARED_STREAMS_HEADER
#define LOG_HAS_DECLARED_STREAMS_HEADER

/* This file extends loguru to enable std::stream-style logging, a la Glog.
   It's an optional feature behind the LOG_WITH_STREAMS settings
...
#undef DCHECK_NE
#undef DCHECK_LT
#undef DCHECK_LE#if defined(__GNUC__) || defined(__clang__)
// Disable all warnings from gcc/clang:
...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	void file_reopen(void* user_data)
	{
		...
		if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || (st.st_ino != file_abs->st.st_ino)) {
			...
			if (!file_abs->fp) {
				VLOG_F(g_internal_verbosity, "Reopening file '" LOG_FMT(s) "' due to previous error", file_abs->path);
			}
			else if (ret < 0) {
				const auto why = errno_as_text();
				VLOG_F(g_internal_verbosity, "Reopening file '" LOG_FMT(s) "' due to '" LOG_FMT(s) "'", file_abs->path, why.c_str());
			}
			else {
				VLOG_F(g_internal_verbosity, "Reopening file '" LOG_FMT(s) "' due to file changed", file_abs->path);
			}
			// try reopen current file.
			if (!create_directories(file_abs->path)) {
				LOG_F(ERROR, "Failed to create directories to '" LOG_FMT(s) "'", file_abs->path);
			}
			file_abs->fp = fopen(file_abs->path, file_abs->mode_str);
			if (!file_abs->fp) {
				LOG_F(ERROR, "Failed to open '" LOG_FMT(s) "'", file_abs->path);
			}
			else {
		...
		static Text vtextprintf(const char* format, va_list vlist)
	{
		...
		CHECK_F(result >= 0, "Bad string format: '" LOG_FMT(s) "'", format);
		return Text(buff);
#endif
	}
		...
		for (int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !std::isalpha(cmd[arg_len], std::locale(""))) {
				...
				if (value_str[0] == '\0') {
					...
					arg_it += 1;
					CHECK_LT_F(arg_it, argc, "Missing verbosiy level after " LOG_FMT(s) "", verbosity_flag);
				...
				if (req_verbosity != Verbosity_INVALID) {
					...
					CHECK_F(end && *end == '\0',
						"Invalid verbosity. Expected integer, INFO, WARNING, ERROR or OFF, got '" LOG_FMT(s) "'", value_str);
				}
			}
			else {
...
#define getcwd _getcwd
		...
		if (!getcwd(s_current_dir, sizeof(s_current_dir))) {
			...
			LOG_F(WARNING, "Failed to get current working directory: " LOG_FMT(s) "", error_text.c_str());
		}
		...
		VLOG_F(g_internal_verbosity, "arguments: " LOG_FMT(s) "", s_arguments.c_str());
		if (strlen(s_current_dir) != 0)
		{
			VLOG_F(g_internal_verbosity, "Current dir: " LOG_FMT(s) "", s_current_dir);
		}
		VLOG_F(g_internal_verbosity, "stderr verbosity: " LOG_FMT(d) "", g_stderr_verbosity);
...
#endif
				if (errno != EEXIST) {
					LOG_F(ERROR, "Failed to create directory '" LOG_FMT(s) "'", file_path);
	...
	bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
	{
		...
		if (!create_directories(path)) {
			LOG_F(ERROR, "Failed to create directories to '" LOG_FMT(s) "'", path);
		}
		...
		if (!file) {
#endif
			LOG_F(ERROR, "Failed to open '" LOG_FMT(s) "'", path);
			return false;
		}
#if LOG_WITH_FILEABS
		...
		VLOG_F(g_internal_verbosity, "Logging to '" LOG_FMT(s) "', mode: '" LOG_FMT(s) "', verbosity: " LOG_FMT(d) "", path, mode_str, verbosity);
		return true;
		}
	...
	/*
		...
		Search for LOG_SYSLOG to find and fix.
	*/
	bool add_syslog(const char* app_name, Verbosity verbosity)
	{
	...
	bool add_syslog(const char* app_name, Verbosity verbosity, int facility)
	{
		...
		VLOG_F(g_internal_verbosity, "Logging to 'syslog' , verbosity: " LOG_FMT(d) "", verbosity);
		return true;
#else
		(void)app_name;
		(void)verbosity;
	...
	bool remove_callback(const char* id)
	{
		...
		if (it != s_callbacks.end()) {
			...
			LOG_F(ERROR, "Failed to locate callback with id '" LOG_FMT(s) "'", id);
			return false;
		}
	}
	...
	void set_thread_name(const char* name)
	{
		...
		strncpy_s(thread_name_buffer(), LOG_THREADNAME_WIDTH + 1, name, _TRUNCATE);
#else // LOG_PTHREADS
	...
	static void print_preamble_header(char* out_buff, size_t out_buff_size)
	{
		...
		if (g_preamble_thread && pos < out_buff_size) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "[%-*s]", LOG_THREADNAME_WIDTH, " thread name/id");
			if (bytes > 0) {
				pos += bytes;
			}
		}
		if (g_preamble_file && pos < out_buff_size) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%*s:line  ", LOG_FILENAME_WIDTH, "file");
			if (bytes > 0) {
				pos += bytes;
			}
		}
...
#if LOG_USE_FMTLIB
			auto buff = textprintf("{:.{}f} s: {:s}", duration_sec, LOG_SCOPE_TIME_PRECISION, _name);
#else
	...
	StreamLogger::~StreamLogger() noexcept(false)
	{
		...
		log(_verbosity, _file, _line, LOG_FMT(s), message.c_str());
	}
		...
		if (!stack.empty()) {
			...
			for (auto entry : stack) {
				...
				auto prefix = textprintf("[ErrorContext] {.{}s}:{:-5u} {:-20s} ",
					filename(entry->_file), LOG_FILENAME_WIDTH, entry->_line, description.c_str());
#else
				auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ",
